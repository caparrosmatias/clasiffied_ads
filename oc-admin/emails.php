            ...
            default:
                ...
                for($i = $start; $i < $max; $i++) {
                    ...
                    if(osc_get_preference('admin_theme') == 'modern') {
                        ...
                        $btn_actions = '<a href="' . osc_admin_base_url(true) . '?page=emails&action=edit&amp;id=' . $email['pk_i_id'] . '" rel="tooltip" class="btn btn-warning" title="' . __('Edit') . '"><i class="material-icons">edit</i><div class="ripple-container"></div></a>';

                        $row[] = $email['s_internal_name'];
                        $row[] = $title['s_title'];
                        $row[] = $btn_actions;
                    }
