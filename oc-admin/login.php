...
class CAdminLogin extends AdminBaseModel
{
    function __construct()
    {
        ...
        if(!osc_get_preference('osclass_activated')) {
            osc_evo_activation('updated');
        }
    }
    ...
    function doModel()
    {
        switch( $this->action ) {
            ...
            case('recover'):        // form to recover the password (in this case we have the form in /gui/)
                $this->doView('gui/' . osc_get_preference('admin_theme') . '_recover.php');
                break;
            case('recover_post'):
                if( defined('DEMO') ) {
                ...
                if(Params::getParam('email')) {
                    ...
                    osc_add_flash_error_message( _m('The E-mail field is empty'), 'admin');
                    $this->redirectTo(osc_admin_base_url(true)."?page=login&action=recover");
                }
                break;
            case('forgot'):         // form to recover the password (in this case we have the form in /gui/)
                ...
                $this->doView( 'gui/' . osc_get_preference('admin_theme') . '_forgot_password.php' );
                break;
            case('forgot_post'):
                osc_csrf_check();
                $admin = Admin::newInstance()->findByIdSecret(Params::getParam('adminId'), Params::getParam('code'));
            ...
            default:
                ...
                $this->doView( 'gui/' . osc_get_preference('admin_theme') . '_login.php' );
                break;
        }
    }
    ...
    {
        $login_admin_title = osc_apply_filter('login_admin_title', 'Osclass Evolution');
        $login_admin_url   = osc_apply_filter('login_admin_url', 'https://osclass-evo.com/');
