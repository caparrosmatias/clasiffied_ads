    ...
    function doModel()
    {
        ...
        switch ($this->action) {
            case 'bulk_actions':
                ...
                switch (Params::getParam('bulk_actions')) {
                    ...
                    case 'delete_all':
                        ...
                        if ($id) {
                            ...
                            foreach ($id as $i) {
                                if ($i) {
                                    $item
                                             = $this->itemManager->findByPrimaryKey(
                                        $i
                                    );
                                    $success = $mItems->delete(
                                        $item['s_secret'],
                                        $item['pk_i_id']
                                    );
                                    if ($success) {
                    ...
                    case 'clear_spam_all';
                        ...
                        if ($id) {
                            ...
                            foreach ($id as $i) {
                                if ($i) {
                                    $success = $this->itemManager->clearStat(
                                        $i,
                                        'spam'
                                    );
                                    if ($success) {
                    ...
                    default:
                        if (Params::getParam("bulk_actions") != "") {
                            osc_run_hook(
                                "item_bulk_" . Params::getParam("bulk_actions"),
                                Params::getParam('id')
                            );
                        }
                        break;
                ...
                $this->redirectTo(
                    Params::getServerParam('HTTP_REFERER', false, false)
                );
                break;
            case 'delete':          //delete
                osc_csrf_check();
                ...
                if ($success) {
                    osc_add_flash_ok_message(
                        _m('The listing has been deleted'),
                        'admin'
                    );
                } else {
                    osc_add_flash_error_message(
                        _m("The listing couldn't be deleted"),
                        'admin'
                    );
                }
                ...
                switch ($value) {
                    ...
                    case 'ACTIVE':
                        ...
                        if ($success && $success > 0) {
                            ...
                            osc_add_flash_error_message(
                                _m(
                                    "The listing can't be activated because it's blocked"
                                ),
                                'admin'
                            );
                        }
            ...
            case 'clear_stat':
                ...
                if ($success) {
                    osc_add_flash_ok_message(
                        _m('The listing has been unmarked as') . " $stat",
                        'admin'
                    );
                } else {
                    osc_add_flash_error_message(
                        _m("The listing hasn't been unmarked as") . " $stat",
                        'admin'
                    );
                }
            ...
            case 'item_edit':       // edit item
                ...
                if ($item['b_active']) {
                    $actions[] = '<a class="btn ' . $float_class . ' btn-sm" href="'
                                 ...
                                 . $item['pk_i_id'] . '&amp;' . $csrf_token
                                 . '&amp;value=INACTIVE">' . __('Deactivate')
                                 . '</a>';
                } else {
                    $actions[] = '<a class="btn btn-red btn-danger ' . $float_class . ' btn-sm" href="'
                                 . osc_admin_base_url(true)
                                 . '?page=items&amp;action=status&amp;id='
                                 . $item['pk_i_id'] . '&amp;' . $csrf_token
                ...
                if (!$item['b_blocked']) {
                    ...
                    $actions[] = '<a class="btn btn-red btn-danger ' . $float_class . ' btn-sm" href="'
                                 ...
                                 . '?page=items&amp;action=status&amp;id='
                                 . $item['pk_i_id'] . '&amp;' . $csrf_token
                                 . '&amp;value=ENABLE">' . __('Unblock')
                                 . '</a>';
                }
                if ($item['b_premium']) {
                    $actions[] = '<a class="btn ' . $float_class . ' btn-sm" href="'
                                 . osc_admin_base_url(true)
                                 . '?page=items&amp;action=status_premium&amp;id='
                                 . $item['pk_i_id'] . '&amp;' . $csrf_token
                                 . '&amp;value=0">' . __('Unmark as premium')
                                 . '</a>';
                } else {
                    $actions[] = '<a class="btn ' . $float_class . ' btn-sm" href="'
                                 . osc_admin_base_url(true)
                                 . '?page=items&amp;action=status_premium&amp;id='
                                 . $item['pk_i_id'] . '&amp;' . $csrf_token
                                 . '&amp;value=1">' . __('Mark as premium')
                                 . '</a>';
                }
                if ($item['b_spam']) {
                    $actions[] = '<a class="btn btn-red btn-danger ' . $float_class . ' btn-sm" href="'
                                 . osc_admin_base_url(true)
                                 . '?page=items&amp;action=status_spam&amp;id='
                                 . $item['pk_i_id'] . '&amp;' . $csrf_token
                                 . '&amp;value=0">' . __('Unmark as spam')
                                 . '</a>';
                } else {
                    $actions[] = '<a class="btn ' . $float_class . ' btn-sm" href="'
                                 . osc_admin_base_url(true)
            ...
            case 'item_edit_post':
                ...
                if ($success == 1) {
                    ...
                    if (Params::getParam('data')) {
                        ...
                        $this->redirectTo(
                            osc_admin_base_url(true)
                            . "?page=items&action=item_edit&id=" . Params::getParam(
                                'id'
                            )
                        );
                    }
            ...
            case 'deleteResource':  //delete resource
                ...
                $result = ItemResource::newInstance()->delete(
                    array(
                        'pk_i_id'      => $id,
                        'fk_i_item_id' => $fkid,
                        's_name'       => $name
            ...
            case 'post_item':
                ...
                if ($success == 1 || $success == 2) {
                    ...
                    osc_add_flash_ok_message(
                        _m('A new listing has been added'),
                        'admin'
                    );
                    ...
                    if (Params::getParam('data')) {
                        ...
                        osc_add_flash_error_message($success, 'admin');
                        $this->redirectTo(
                            osc_admin_base_url(true) . "?page=items&action=post"
                        );
                    }
                }
                break;
            ...
            case('settings_post'):     // update item settings
                ...
                $enabledRecaptchaItems    = (($enabledRecaptchaItems == '1')
                    ? true : false);
                $moderateItems            = Params::getParam('moderate_items');
                $moderateItems            = (($moderateItems != '') ? true
                    : false);
                $numModerateItems         = Params::getParam(
                    'num_moderate_items'
                );
                $itemsWaitTime            = Params::getParam('items_wait_time');
                $loggedUserItemValidation = Params::getParam(
                    'logged_user_item_validation'
                );
                $loggedUserItemValidation = (($loggedUserItemValidation != '')
                    ? true : false);
                $regUserPost              = Params::getParam('reg_user_post');
                $regUserPost              = (($regUserPost != '') ? true
                    : false);
                $notifyNewItem            = Params::getParam('notify_new_item');
                $notifyNewItem            = (($notifyNewItem != '') ? true
                    : false);
                $notifyContactItem        = Params::getParam(
                    'notify_contact_item'
                );
                $notifyContactItem        = (($notifyContactItem != '') ? true
                    : false);
                $notifyContactFriends     = Params::getParam(
                ...
                $enabledFieldPriceItems   = (($enabledFieldPriceItems != '')
                    ? true : false);
                $enabledFieldImagesItems  = Params::getParam(
                    'enableField#images@items'
                );
                $enabledFieldImagesItems  = (($enabledFieldImagesItems != '')
                    ? true : false);
                $numImagesItems           = Params::getParam('numImages@items');
                if ($numImagesItems == '') {
                    $numImagesItems = 0;
                }
                $regUserCanContact     = Params::getParam(
                    'reg_user_can_contact'
                );
                $regUserCanContact     = (($regUserCanContact != '') ? true
                    : false);
                $contactItemAttachment = Params::getParam('item_attachment');
                $contactItemAttachment = (($contactItemAttachment != '') ? true
                    : false);
                $warnExpiration        = Params::getParam('warn_expiration');
                $warnExpiration        = (int)$warnExpiration;
                $titleLength           = Params::getParam(
                    'max_chars_per_title'
                );
                $descriptionLength     = Params::getParam(
                ...
                if ( ! osc_validate_int(Params::getParam("items_wait_time"))) {
                    $msg .= _m("Wait time must only contain numeric characters")
                            . "<br/>";
                }
                if (Params::getParam("num_moderate_items") != ''
                    && ! osc_validate_int(
                        Params::getParam("num_moderate_items")
                    )
                ) {
                    $msg .= _m(
                                "Number of moderated listings must only contain numeric characters"
                            ) . "<br/>";
                }
                if ( ! osc_validate_int($numImagesItems)) {
                    $msg .= _m(
                                "Images per listing must only contain numeric characters"
                            ) . "<br/>";
                }
                if ( ! osc_validate_int($warnExpiration)) {
                    $msg .= _m(
                                "Number of expiration days has to be a numeric value"
                            ) . "<br/>";
                }
                if ( ! osc_validate_int($titleLength)) {
                    $msg .= _m("Title Length has to be a numeric value")
                            . "<br/>";
                }
                if ( ! osc_validate_int($descriptionLength)) {
                    $msg .= _m("Description Length has to be a numeric value")
                            . "<br/>";
                }
                if ($msg != '') {
                ...
                $iUpdated += osc_set_preference(
                    'enableField#f_price@items',
                    $enabledFieldPriceItems
                );
                $iUpdated += osc_set_preference(
                    'enableField#images@items',
                    $enabledFieldImagesItems
                );
                $iUpdated += osc_set_preference(
                    'items_wait_time',
                    $itemsWaitTime
                ...
                if ($iUpdated > 0) {
                    osc_add_flash_ok_message(
                        _m("Listings' settings have been updated"),
                        'admin'
                    );
                }
                $this->redirectTo(
            ...
            case('items_reported'):
                ...
                if (Params::getParam('iDisplayLength') != '') {
                    ...
                    if (Cookie::newInstance()->get_value(
                            'listing_iDisplayLength'
                        ) != ''
                    ) {
                        Params::setParam(
                            'iDisplayLength',
                            Cookie::newInstance()->get_value(
                                'listing_iDisplayLength'
                            )
                        );
                    } else {
                ...
                $this->_exportVariableToView(
                    'iDisplayLength',
                    Params::getParam('iDisplayLength')
                );
