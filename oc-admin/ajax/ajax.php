        ...
        switch ($this->action) {
            ...
            case 'enable_category':
                ...
                if($aCategory['fk_i_parent_id'] == '') {
                    ...
                    $subCategories = $mCategory->findAllSubcategories($id);

                    if(count($subCategories)) {
                        foreach($subCategories as $subId => $subcategory) {
                            $mCategory->update(array('b_enabled' => $enabled), array('pk_i_id' => $subId));

                            $aIds[] = $subId;
                            $aUpdated[] = array('id' => $subId);
                        }
                    }
                ...
                // subcategory
                $parentCategory = $mCategory->findParentCategory($id);
            ...
            case 'order_pages':
                ...
                break;
            case 'check_version':
                if(osc_need_core_update(false)) {
                    $data = array('version' => osc_get_latest_core_version(false));
                    $json = json_encode($data);

                    osc_set_preference('update_core_json', $json);
...
//                    }
            ...
            case 'check_plugins':
                ...
                break;
            case 'db_informer':
                $dump = json_decode(Params::getParam('log'), true);

                if(!file_exists($dump['file_job'])) exit;
                require_once $dump['file_job'];

                if (!file_exists($dump['file_rtl'])) exit;
                $file_rtl = fopen($dump['file_rtl'], 'r+b');
                
                $time = time();
                $parse_rtl = explode("\t", fgets($file_rtl));

                if(empty($f[0])) $parse_rtl[0] = $time;

                $file_log = fopen($dump['file_log'], 'rb');

                $log_seek = filesize($dump['file_log']);

                $raw_log = fread($file_log, $log_seek);
                $log = '';
                $logs = array();
                ...
                if(!empty($raw_log)){
                    ...
                    if(!empty($logs)) $log = "['" .implode("', '", $logs). "']";
                }
                ...
                if($parse_rtl[4] == 'EOJ') {
                    ...
                    $res = array(
                        'logs'       => $log,
                        'rows_count' => count($logs),
                        'records'    => $parse_rtl[10],
                        'file_size'  => $parse_rtl[3]
                    );

                    echo json_encode($res);

                    unlink($dump['file_log']);
                    unlink($dump['file_rtl']);
                    unlink($dump['file_job']);
                }
                break;
            case 'script_informer':
                osc_csrf_check();

                $dir = osc_base_path();

                $files = osc_scan_dir($dir);
            ...
            case 'script_backup':
                ...
                $archive_dir = osc_base_path();
                $path = osc_content_path() . 'downloads/oc-backup/';
                $file_name = 'backup_' . date('Y-m-d_H-i-s') . '.zip';

                $archive_name = $path . $file_name;

                if(osc_zip_folder($archive_dir, $archive_name)) {
                    echo filesize($archive_name);
                }
                break;
            case 'db_backup':
                osc_csrf_check();
                ...
                $path = osc_content_path() . 'downloads/oc-backup/';
                $file_name = 'dump_' . osc_db_name() . '_' . date('Y-m-d_H-i-s') . '.sql';

                osc_dbdump($path, $file_name);

                $file_dump = implode("", file($path . $file_name));

                $gzdata = gzencode($file_dump, 9);
                $fp = fopen($path . $file_name . '.gz', "w");
                fwrite($fp, $gzdata);
                fclose($fp);

                @unlink($path . $file_name);

                $dump = Dump::newInstance()->dump_log($path . $file_name . '.gz');
             ...
             ******************************/
            case 'core-upgrade': // AT THIS POINT WE KNOW IF THERE'S AN UPDATE OR NOT
                osc_csrf_check();
...
//                    $result = osc_do_upgrade();
                ...
                osc_upgrade_download_completed();
                break;
            case 'core-upgrade-extract':
                $file_name = Params::getParam('file');

                $tmp_path = osc_content_path() . 'downloads/oc-temp/core-' . osc_get_latest_core_version(true) . '/';

                @mkdir(osc_content_path() . 'downloads/oc-temp/', 0777);
                @mkdir($tmp_path, 0777);

                if($res = osc_unzip_file(osc_content_path() . 'downloads/oc-upgrade/' . $file_name, $tmp_path)) {
                    $result = array('status' => 'success', 'file' => $file_name);
                } else {
...
//                            $data = osc_copy($tmp_path . $_file, osc_content_path() . 'downloads/oc-temp/updated/' . $_file);
                    ...
                    @unlink(osc_content_path() . 'downloads/oc-upgrade/' . $file_name);
                    ...
                    if($fail == 0) {
                        ...
                        if ($rm_errors == 0) {
                            $message = __('Installation completed!');

                            $result = array('status' => 'success', 'message' => $message);
                        } else {
                            $message = __('Nearly everything looks good! Your Osclass Evolution installation is up-to-date, but there were some errors removing temporary files. Please manually remove the "oc-content/downloads/oc-temp" folder');
                ...
                echo json_encode($result);
                break;
            case 'core-upgrade-remind-later':
                osc_set_preference('core_upgrade_remind_later', date('Y-m-d H:i:s', strtotime('+1 day')));
                break;
            /*******************************
