...
function osc_check_recaptcha() {
    ...
    } else if(osc_recaptcha_version() == '3') {
        if (Params::getParam("g-recaptcha-response")) {
            return osc_recaptcha3_validate(Params::getParam("g-recaptcha-response"));
        }
    }
...
function osc_csrfguard_replace_forms($form_data_html) {
    ...
    if(is_array($matches)) {
        foreach ($matches as $m) {
            ...
            $form_data_html = str_replace($m[0], "<form{$m[1]}>" . $m[2] . osc_csrf_token_form() . "</form>", $form_data_html);
        }
    }
