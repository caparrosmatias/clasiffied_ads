        ...
        if(!empty($users)) {
            ...
            foreach($users as $aRow) {
                ...
                if(osc_get_preference('admin_theme') == 'modern') {
                    ...
                    // first column
                    if($aRow['b_active']) {
                        $options_more[] = '<a class="dropdown-item"  href="' . osc_admin_base_url(true) . '?page=users&action=deactivate&amp;id[]=' . $aRow['pk_i_id'] . '&amp;' . $csrf_token_url . '">' . __('Deactivate') .'</a>';
                    } else {
                        $options_more[] = '<a class="dropdown-item"  href="' . osc_admin_base_url(true) . '?page=users&action=activate&amp;id[]=' . $aRow['pk_i_id'] . '&amp;' . $csrf_token_url . '">' . __('Activate') .'</a>';
                    }
                    if($aRow['b_enabled']) {
                        $options_more[] = '<a class="dropdown-item" href="' . osc_admin_base_url(true) . '?page=users&action=disable&amp;id[]=' . $aRow['pk_i_id'] . '&amp;' . $csrf_token_url . '">' . __('Block') .'</a>';
                    } else {
                        $options_more[] = '<a class="dropdown-item" href="' . osc_admin_base_url(true) . '?page=users&action=enable&amp;id[]=' . $aRow['pk_i_id'] . '&amp;' . $csrf_token_url . '">' . __('Unblock') .'</a>';
                    }
                    if( osc_user_validation_enabled() && ( $aRow['b_active'] == 0 ) ) {
                        $options_more[] = '<a class="dropdown-item" href="' . osc_admin_base_url(true) . '?page=users&action=resend_activation&amp;id[]=' . $aRow['pk_i_id'] . '&amp;' . $csrf_token_url . '">' . __('Re-send activation email') . '</a>';
                    }
                    ...
                    $btn_actions = '<a href="' . osc_admin_base_url(true) . '?page=users&action=edit&amp;id=' . $aRow['pk_i_id'] . '" rel="tooltip" class="btn btn-warning" title="' . __('Edit') . '"><i class="material-icons">edit</i><div class="ripple-container"></div></a>';

                    $btn_actions .= '<a id="listing-delete" data-delete-type="user" data-listing-id="' . $aRow['pk_i_id'] . '" href="' . osc_admin_base_url(true) . '?page=users&amp;action=delete&amp;id[]=' . $aRow['pk_i_id'] . '" rel="tooltip" class="btn btn-danger" title="' . __('Delete') . '"><i class="material-icons">delete</i><div class="ripple-container"></div></a>';

                    $btn_actions .= '<a href="' . osc_user_public_profile_url($aRow['pk_i_id']) . '" rel="tooltip" class="btn btn-light" title="' . __('Public profile') . '" target="_blank"><i class="material-icons">person</i><div class="ripple-container"></div></a>';
